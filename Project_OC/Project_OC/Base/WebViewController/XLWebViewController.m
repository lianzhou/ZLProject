...
@property (nonatomic, strong) NSMutableArray   * imgSrcArrayM;
@end
...
-(void)initWKWebView
{
    ...
    if (self.navigationController && self.isHidenNaviBar == NO) {
        f = CGRectMake(0, 0, self.view.bounds.size.width, kScreenHeight - ZLNavBarHeight);
    }
    
    _webView = [[WKWebView alloc]initWithFrame:f configuration:configuration];
    _webView.navigationDelegate = self; //设置WKNavigationDelegate代理关联本类
...
-(void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    if ([url.absoluteString hasPrefix:@"myweb:imageClick:"]) {
        // return NO;
        NSString *imageUrl = [url.absoluteString substringFromIndex:@"myweb:imageClick:".length];
        
        //预览图片
        NSInteger number=1;
        if(  self.imgSrcArrayM.count>0){
            number = [self.imgSrcArrayM indexOfObject:imageUrl];
        }
        ...
        [webView evaluateJavaScript:str  completionHandler:^(id Result, NSError * error) {
            if( weakSelf.imgSrcArrayM.count>0){
                ...
                imageBrowser.bigImageArray = self.imgSrcArrayM.count>0?self.imgSrcArrayM: @[imageUrl];
