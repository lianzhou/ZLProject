...
- (instancetype)initWithFrame:(CGRect)frame withTapBlock:(TapBlock)tapBlock
{
    if (self = [super initWithFrame:frame]) {
        ...
        CGFloat btnW = 60;
        CGFloat btnH = 30;
        _countBtn = [[UIButton alloc] initWithFrame:CGRectMake(kScreenW - btnW - 24, ZLNavBarHeight, btnW, btnH)];
        ...
        NSString *filePath = [self getFilePathWithImageName:[ZLUserDefaults valueForKey:adImageName]];
...
- (void)downloadAdImageWithUrl:(NSString *)imageUrl imageName:(NSString *)imageName
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if ([UIImagePNGRepresentation(image) writeToFile:filePath atomically:YES]) {// 保存成功
            NSLog(@"广告页保存成功");
            [self deleteOldImage];
            [ZLUserDefaults setValue:imageName forKey:adImageName];
...
- (void)deleteOldImage
{
    NSString *imageName = [ZLUserDefaults valueForKey:adImageName];
    if (imageName) {
        NSString *filePath = [self getFilePathWithImageName:imageName];
        NSFileManager *fileManager = [NSFileManager defaultManager];
