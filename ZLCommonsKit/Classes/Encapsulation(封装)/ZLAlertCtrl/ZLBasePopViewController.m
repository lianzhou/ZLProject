...
@interface ZLBasePopViewController ()<UIViewControllerTransitioningDelegate>
//默认不加载
...
@implementation ZLBasePopViewController
- (instancetype)init
{
    self = [super init];
    if (self) {
...
- (UIView *)blurBackgroundView {
    if (self.blurBackground) {
        if (!_blurBackgroundView) {
            ...
            [_blurBackgroundView zl_setTarget:self action:@selector(dismissViewControllerAnimated)];
        }
    } else {
        if (!_blurBackgroundView) {
