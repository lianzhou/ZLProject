...
- (void)setTagsArray:(NSArray *)tagsArray
{
    ...
    for (NSInteger i=0; i<tagsArray.count; i++) {
        ...
        btnW = [self sizeWithText:tagsArray[i] font:KDEFAULTFONT(14)].width + _tagsMinPadding * 2;
        
        if (i < tagsArray.count-1) {
            nextWidth = [self sizeWithText:tagsArray[i+1] font:KDEFAULTFONT(14)].width + _tagsMinPadding * 2;
        }
        
        CGFloat nextBtnX = btnX + btnW + _tagsMargin;
        
        // 如果下一个按钮，标签最右边则换行
        if ((nextBtnX + nextWidth) > (_tagsFrameWidth - _tagsMargin)) {
            // 计算超过的宽度
            moreWidth = _tagsFrameWidth - nextBtnX;
    ...
    for (NSInteger i=0; i<lastIndexs.count; i++) {
        ...
        for (NSInteger j=0; j<newArr.count; j++) {
            ...
            tagW = [self sizeWithText:newArr[j] font:KDEFAULTFONT(14)].width + _tagsMinPadding * 2 + averageW;
